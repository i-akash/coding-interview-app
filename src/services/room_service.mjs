...
RoomService.prototype.onJoinRoom = function () {
    this.socket.on(JOIN_ROOM, (joinInfo, ackCallback) => {
        ...
        this.redisClient.get(`${userInfo.userRoom}`).then(room => {
            room = JSON.parse(room)
            let roomIsExist = !!room
            let candidate = userInfo.userRole === "Candidate"

            if (roomIsExist === false && candidate) {
                ackCallback("This room doesn't exist and candidate is not allowed to create one")
                return
            }
            if (candidate && !!room.candidate && room.candidate !== userInfo.userName) {
            ...
            if (roomIsExist === false) {
                room = { name: userInfo.userRoom, owner: userInfo.userName, password: joinInfo.roomPassword }
                this.redisClient.set(`${userInfo.userRoom}`, JSON.stringify(room))
                this.redisClient.rpush(["rooms", JSON.stringify({ name: userInfo.userRoom, private: !!joinInfo.roomPassword })], redis.print)
            }
...
RoomService.prototype.onDisconnecting = function () {
    this.socket.on(DISCONNECTING, () => {
        ...
        this.redisClient.get(socketId).then(userInfo => {
...
RoomService.prototype.getRoomInfo = function () {
    this.socket.on(ROOM_INFO, (room, callback) => {
        this.redisClient.get(`${room}`).then(info => callback(JSON.parse(info))).catch(error => callback({}))
    })
}
...
RoomService.prototype.onChangeRoomInfo = function () {
    this.socket.on(CHANGE_ROOM_INFO, ({ room, payload: info }, callback) => {
        ...
        this.redisClient.set(`${room}`, JSON.stringify(info)).then(resp => {
